  ...
  try {
    ...
    if (!branch || !newBranchName) {
      return NextResponse.json(
        errorResponse(400, "Both old and new branch names are required"),
        {
          status: 400,
        }
      );
    ...
    return NextResponse.json(
      successResponse(200, updatedBranch, "Branch updated successfully"),
      {
        status: 200,
      }
    );
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json(
      successResponse(200, null, "Branch deleted successfully"),
      {
        status: 200,
      }
    );
