...
export async function GET(
  ...
  NextResponse<ApiResponse<Admin | Faculty | Student | Staff | null>>
> {
  try {
    const { id } = params;
    ...
    return NextResponse.json(
      successResponse(
        ...
        `${userType} profile fetched successfully`
      ),
      { status: 200 }
    );
  } catch (err) {
...
> {
  try {
    ...
    if (!body || Object.keys(body).length === 0) {
      return NextResponse.json(
        errorResponse(400, "Request body cannot be empty"),
        { status: 400 }
      );
    }
    ...
    if (!userType) {
      return NextResponse.json(errorResponse(400, "User type is required"), {
        status: 400,
      });
    }
    ...
    switch (userType) {
      ...
      default:
        return NextResponse.json(errorResponse(400, "Invalid user type"), {
          status: 400,
        });
    }
    ...
    if (!updatedUser) {
      return NextResponse.json(
        errorResponse(404, "User not found or could not be updated"),
        { status: 404 }
      );
    }
    ...
    return NextResponse.json(
      successResponse(
        200,
        serializeBigInt(updatedUser),
        `${userType} profile updated successfully`
      ),
      { status: 200 }
    );
  } catch (err) {
...
export async function DELETE(
  ...
  try {
    ...
    deletedUser = await prisma.admin
      .delete({ where: { id } })
      .catch(() => null);
    if (deletedUser) userType = "Admin";

    if (!deletedUser) {
      deletedUser = await prisma.student
        .delete({ where: { id } })
        .catch(() => null);
    ...
    if (!deletedUser) {
      deletedUser = await prisma.faculty
        .delete({ where: { id } })
        .catch(() => null);
    ...
    if (!deletedUser) {
      deletedUser = await prisma.staff
        .delete({ where: { id } })
        .catch(() => null);
    ...
    return NextResponse.json(
      successResponse(200, deletedUser, `${userType} deleted successfully`),
      { status: 200 }
    );
  } catch (err) {
    const error = err instanceof Error ? err.message : String(err);
