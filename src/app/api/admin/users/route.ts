...
> {
  try {
    ...
    const { userType, password, ...userData } = body; // Extracting user type (Admin, Student, Faculty, Staff)
    ...
    if (!password) {
      return NextResponse.json(errorResponse(400, "Password is required"), {
        status: 400,
      });
    }
