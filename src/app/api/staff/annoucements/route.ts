    ...
    try {
      ...
      const filteredAnnouncements = announcements.filter((announcement) => {
        ...
        if (Object.keys(filter).length === 0) {
          return announcement.role === "STAFF";
        }
        // role checking
        if (announcement.role !== "STAFF") {
          return false;
        }
...
export async function POST(
  ...
  try {
    ...
    if (role !== "STUDENT" && role !== "STAFF" && role !== "FACULTY") {
...
export async function PATCH(req: NextRequest): Promise<NextResponse<ApiResponse<Announcement | null>>> {
  try {
    ...
    if (findAnnouncement.role !== "STAFF" && findAnnouncement.role !== "STUDENT" && findAnnouncement.role !== "FACULTY") {
